document.addEventListener("DOMContentLoaded", function () {
    ...
    function updateHealthAlert(data) {
        ...
        if (data.glucose) {
            ...
            if (color === 'red' || color === 'orange') {
                const alertType = getAlertType(color);
                alerts.push({
                    type: alertType,
    ...
    // Função para mostrar os alertas de saúde
    function showHealthAlerts(alerts) {
        const healthAlertSection = document.querySelector('.health-alert-section');

        // Priorizar alertas críticos (vermelho > laranja > amarelo > azul)
        const priorityOrder = { red: 4, orange: 3, yellow: 2, lightblue: 1 };
        alerts.sort((a, b) => priorityOrder[b.color] - priorityOrder[a.color]);

        // Definir cor de fundo baseada no alerta de maior prioridade
        const primaryAlert = alerts[0];
        const bgColor = getAlertBackgroundColor(primaryAlert.color);

        // Aplicar a cor de fundo dinamicamente
        healthAlertSection.className = `mb-6 rounded-2xl p-4 text-white shadow-sm health-alert-section ${bgColor}`;

        // Mostrar o alerta de maior prioridade ou combinar se houver múltiplos
        let alertHTML;

        if (alerts.length === 1) {
            alertHTML = createSingleAlert(primaryAlert);
        } else {
            // Se há múltiplos alertas, mostrar o mais crítico com indicação de quantos são
            const totalAlerts = alerts.length;
            alertHTML = createMultipleAlertsDisplay(primaryAlert, totalAlerts, alerts);
        }

        healthAlertSection.innerHTML = alertHTML;
        healthAlertSection.style.display = 'block';
    }

    // Função para criar display de um único alerta
    function createSingleAlert(alert) {
        ...
        return `
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-base font-bold">${alert.type}</h3>
    ...
    // Função para criar display de múltiplos alertas
    function createMultipleAlertsDisplay(primaryAlert, totalAlerts, allAlerts) {
        const bgColor = getAlertBackgroundColor(primaryAlert.color);

        // Criar lista de todas as mensagens
        const allMessages = allAlerts.map(alert => alert.message).join(' • ');
        ...
        return `
            <div class="flex items-center gap-3">
                ...
                <div class="flex-1">
                    <h3 class="text-base font-bold">Múltiplos Alertas (${totalAlerts})</h3>
                    <p class="text-sm">${allMessages}</p>
                </div>
            </div>
        `;
    }
    ...
    async function fetchDailySummary() {
        try {
            ...
            const response = await fetch(`http://localhost:3000/api/health/daily-summary?user_id=${userId}&date=${currentDate}`);
